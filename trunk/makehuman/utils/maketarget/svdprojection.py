...
def get_target_vector(obj,vertex_list):
    #print mbase.T.shape, mbase.shape, matrix(targ).T.shape
...
def projectTarget(base,vertex_list,lookup):
   ...
   # Computing projection
    targ = get_target_vector(obj,vertex_list)    
    proj = apply_projection(base,targ)
    ...
    for i in vertex_list :
        ...
        v = obj.verts[i]
        v.co[0] = originalVertex[0]+delta[0]
        v.co[1] = originalVertex[1]+delta[1]
        v.co[2] = originalVertex[2]+delta[2]
    obj.update()
    obj.calcNormals()
...
def b_event(event):
    ...
    """
    global symmPath,selAxis,morphFactor,targetVertexList
    if event == 0: pass
    elif event == 1:
