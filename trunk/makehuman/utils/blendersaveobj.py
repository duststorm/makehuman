...
def saveObj(path,prefix="no"):
    ...
    #Move special groups at the end of the vertsGroups
    specialGroups = set()
    specialNames = ("lash","eyebrown","cornea")
    print "---"

    for sName in specialNames:
        for g in vertsGroups:
            words = g.split('-')
            if sName in words:
                specialGroups.add(g)

    for sGroup in specialGroups:
        vertsGroups.remove(sGroup)
        vertsGroups.append(sGroup)
